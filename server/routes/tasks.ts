...
router.get('/', (req: Request, res: Response) => {
  ...
  if (!projectId) return res.status(400).json({ error: 'errors.projectIdRequired' });
...
router.post('/', (req: Request, res: Response) => {
  ...
  if (!name || !project_id) return res.status(400).json({ error: 'errors.nameAndProjectRequired' });
